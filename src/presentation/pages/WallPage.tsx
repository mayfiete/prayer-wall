    ...
    <div className="min-h-screen flex flex-col bg-stone-100 font-body">
      ...
      <header className="flex items-center gap-4 px-8 py-6 bg-white border-b border-stone-200">
        ...
        <div className="flex-1">
          <h1 className="font-sans text-[26px] font-semibold text-[var(--color-heading)] leading-tight tracking-tight" style={{ fontFamily: 'var(--font-heading)' }}>{wallTitle}</h1>
          <p className="text-sm text-[var(--color-muted)] mt-0.5">{ORG_NAME}</p>
        </div>
      </header>
        ...
        {categories.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {categories.map((cat) => (
              <span
                key={cat.id}
                className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-[#f4f0f4] text-[var(--color-heading)] border border-[#d9d9d9]"
              >
                {cat.name}
              </span>
            ))}
      ...
      <section className="flex-1 px-0 pb-10 bg-[var(--color-background)] overflow-x-clip">
