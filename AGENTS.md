...
- Migration `033_restrict_donation_public_reads.sql` repairs donation privacy: revoke table-level SELECT from PUBLIC, anon, and authenticated, then grant only the explicit public projection (including `processor`). A column REVOKE cannot override table-level SELECT. Do not rerun migration 024 after 033. Browser queries must list public columns, never `*`.
- Database regression tests: set `DONATION_TEST_CONTAINER` to a fresh disposable PostgreSQL Docker container with logical replication enabled, then run `node --test supabase/functions/donation-persistence.test.mjs`. The test creates roles and schemas; never point it at a real project.
- API/Realtime privacy tests: use a fresh disposable local Supabase CLI stack with `prayer_wall` in its API schemas. Set `DONATION_ACCESS_TEST_CONTAINER` to its `supabase_db_...` container, `DONATION_ACCESS_TEST_URL` to its loopback API URL, `DONATION_ACCESS_TEST_ANON_KEY` to its anon key, and `DONATION_ACCESS_TEST_JWT_SECRET` to that local stack's JWT secret; run `node --test supabase/functions/donation-access.test.mjs`. Tests create synthetic donations directly (no Stripe calls or emails), verify browser/service-role API access, and test existing and new Realtime subscriptions. Never use production credentials.
